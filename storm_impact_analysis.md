...
The purpose of this anaylsis is to determine which types of storms and weather events are most harmful with respect to population health and which have the greatest economic consequences. Analyzing storm data from the National Oceanic and Atmospheric Administration can help answer these questions. This document details step-by-step analysis so that the reader may reproduce it in its entirety.

### Analytical Approach

To answer the question, "Which events are most harmful to population health?", the analysis below simply sums up the fatalities and injuries by type of event and sort in decreasing order of the number of fatalities and injuries. 

Similarly, to answer the question, "Which events have the greatest economic consequences?", the analysis below simply sums the crop damage and property damage by the type of event and sort in decreasing order of total damage value. 
...
Set global parameters and identify the R packages that were used in this analysis
...
For faster and easier processing, drop unused columns and format data into a data table
...
To answer the two main questions, calculate the overall sums of each measure for each event category. While the data extend as far back as 1950, much more data is available in recent years (since 1995). As a result, the sums predominantly reflect the time period since then.
...
To answer the forward-looking question from the perspective of the municipal manager, we need a bit more information:
...
11. Estimated 12 month fatality for each event type _(annual avg # of events x avg fatalities per event)_  
12. Estimated 12 month injury count for each event type _(annual avg # of events x avg injuries per event)_  
13. Estimated 12 month crop damage for each event type _(annual avg # of events x avg crop damage per event)_  
14. Estimated 12 month property damage for each event type _(annual avg # of events x avg property damage per event)_
...
totalCountsByEventCategory <- eventCountsByYear[, list(totalEventCount = sum(freq)), 
                                                by = c("EventCategory")]

avgCountsByEventCategory <- eventCountsByYear[, list(avgAnnualEventCount = mean(freq)), 
                                              by = c("EventCategory")]

# merge
stormDataByEventCategory <- merge(x = stormDataByEventCategory, 
                                  y = avgCountsByEventCategory, 
                                  by = c("EventCategory"), 
                                  all = FALSE)

stormDataByEventCategory <- merge(x = stormDataByEventCategory, 
                                  y = totalCountsByEventCategory, 
                                  by = c("EventCategory"), 
                                  all = FALSE)
```
...
Calculate estimates of each measure for the next 12 months
Estimated NTM Measure = Avg Measure Per Event (x) Avg Annual Event Count
...
#### Results of Analysis from Municipal Manager's Perspective

From the perspective of the municipal manager, the analysis below highlights a prioritization list for the next year based on the average number of events likely to occur in a given year and the average impact of each event in that event category. Their product produces metrics for estimated human harm and estimated economic damage over the next 12 months.

These results show that `tornadoes`, `floods`, `thunderstorms`, and `excessive heat` are the event categories that are most likely to cause the greatest human or economic harm in a given year and ought to be prioritized in the municipal manager's planning efforts.
